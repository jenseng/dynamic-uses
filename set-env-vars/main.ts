...
function main() {
  ...
  if (!['overwrite', 'preserve', 'error'].includes(onConflict)) {
    core.setFailed('on-conflict input must be one of: overwrite, preserve, error');
    return;
  }
  ...
  for (const [key, value] of Object.entries(variables)) {
    ...
    if (keyString in process.env) {
      if (onConflict === 'error') {
        ...
        core.warning(`Environment variable "${keyString}" already exists, overwriting with new value`);
      } else if (onConflict === 'preserve') {
        core.warning(`Environment variable "${keyString}" already exists, preserving existing value`);
        continue;
      }
    }
